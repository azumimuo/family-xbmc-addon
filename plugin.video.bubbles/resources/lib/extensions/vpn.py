...
class Vpn(object):
	...
	def _verification(self, loader = False):
		try:
			...
			return (not informationAddress.startswith(self._mask(mask)), informationAddress, informationProvider, informationLocation)
		except:
			tools.Logger.error()
			return (False, '', '', '')
	...
	def launch(self, execution):
		...
		if self._launchEnabled(execution = execution):
			threading.Thread(target = self._launch, args = (execution,)).start()
