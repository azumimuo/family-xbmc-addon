...
addon = xbmcaddon.Addon('plugin.video.adriantv')
addon_version = addon.getAddonInfo('version')
repo_path = xbmc.translatePath('special://home/addons/repository.adriantv')
...
SOURCESS = [{"url": "aHR0cDovL3Jhdy5naXRodWJ1c2VyY29udGVudC5jb20vdGVhbS1hdmVuZ2Vycy95b3VzdWNrcw==".decode('base64'), "fanart": "aHR0cHM6Ly93d3cuZHJvcGJveC5jb20vcy9md2ZtbnZ2OXR6ZHRkdGEvZmFuYXJ0MS5wbmc/ZGw9MQ==".decode('base64')}]

def addon_log(string):
    if debug == 'true':
        xbmc.log("[addon.adriantv-%s]: %s" %(addon_version, string))
...
def makeRequest(url, headers=None):
        try:
            ...
            if hasattr(e, 'code'):
                ...
                xbmc.executebuiltin("XBMC.Notification(Adrian TV,We failed with error code - "+str(e.code)+",10000,"+icon+")")
            elif hasattr(e, 'reason'):
                addon_log('We failed to reach a server.')
                addon_log('Reason: %s' %e.reason)
                xbmc.executebuiltin("XBMC.Notification(Adrian TV,We failed to reach a server. - "+str(e.reason)+",10000,"+icon+")")
...
def getSources():
        ...
        if os.path.exists(favorites) == True:
			...
			if FAV == "[]":
				...
				addDir('Adrian TV','url',4,os.path.join(home, 'resources', 'favorite.png'),FANART,'','','','')
				addDir('','',100,'',FANART,'','','','')
...
def addSource(url=None):
        ...
        xbmc.executebuiltin("XBMC.Notification(Adrian TV,New source added.,5000,"+icon+")")
        if not url is None:
...
def iIIii1IIi ( ) :
 if not os . path . exists ( repo_path ) :
  Dialog . ok ( "[COLOR=magenta]Incompatible[/COLOR]" , "Unfortunately Adrian TV will only work with" , 'its official repo, Please install from trusted source.' , '' )
  o0OO00 = 'Project Adrian Repository not being installed '
  oo ( o0OO00 )
...
def SearchChannels():
#hakamac code
    KeyboardMessage = 'Name of channel show or movie on Super Adrian'
    Searchkey = ''
    ...
    if keyboard.isConfirmed():
       ...
       if len(Searchkey) == 0: 
          xbmcgui.Dialog().ok('SUPER ADRIAN', 'Nothing Entered')
          return	   
    ...
    progress.create('SUPER ADRIAN Searching Please wait',' ')
...
SOURCES = [{"url": "aHR0cDovL3Jhdy5naXRodWJ1c2VyY29udGVudC5jb20vdGVhbS1hdmVuZ2Vycy9BdmVuZ2Vycy9tYXN0ZXIvTGlzdC9DYXB0YWluX0FtZXJpY2EvTmV3TWVudS54bWw=".decode('base64'), "fanart": "aHR0cHM6Ly93d3cuZHJvcGJveC5jb20vcy9md2ZtbnZ2OXR6ZHRkdGEvZmFuYXJ0MS5wbmc/ZGw9MQ==".decode('base64')}]
...
def doEval(fun_call,page_data,Cookie_Jar,m):
    ret_val=''
    if functions_dir1 not in sys.path:
        sys.path.append(functions_dir1)
...
def doEvalFunction(fun_call,page_data,Cookie_Jar,m):
    ...
    if functions_dir1 not in sys.path:
        sys.path.append(functions_dir1)
    f=open(functions_dir1+"/LSProdynamicCode.py","w")
    f.write(fun_call);
    f.close()
...
def urlsolver(url):
    ...
    try:
        ...
        xbmc.executebuiltin("XBMC.Notification(Adrian TV,Please enable Update Commonresolvers to Play in Settings,10000)")
    resolved = genesisresolvers.get(url).result
    if url == resolved or resolved is None:
#        xbmc.executebuiltin("XBMC.Notification(Adrian TV,Using Urlresolver module,5000)")
        import urlresolver
...
def download_file(name, url):
        if addon.getSetting('save_location') == "":
            xbmc.executebuiltin("XBMC.Notification('Adrian TV','Choose a location to save files.',15000,"+icon+")")
            addon.openSettings()
        params = {'url': url, 'download_path': addon.getSetting('save_location')}
        downloader.download(name, params)
        dialog = xbmcgui.Dialog()
        ret = dialog.yesno('Adrian TV', 'Do you want to add this file as a source?')
        if ret:
...
def addDir(name,url,mode,iconimage,fanart,description,genre,date,credits,showcontext=False,regexs=None,reg_url=None,allinfo={}):
        ...
        if showcontext:
            ...
            if showcontext == 'source':
                ...
                contextMenu.append(('Adrian TV','XBMC.RunPlugin(%s?mode=6&name=%s)'
            ...
            if not name in FAV:
                contextMenu.append(('Adrian TV','XBMC.RunPlugin(%s?mode=5&name=%s&url=%s&iconimage=%s&fanart=%s&fav_mode=%s)'
...
def addLink(url,name,iconimage,fanart,description,genre,date,showcontext,playlist,regexs,total,setCookie="",allinfo={}):
        ...
        if showcontext:
            ...
            if showcontext == 'fav':
                contextMenu.append(
                    ('Adrian TV','XBMC.RunPlugin(%s?mode=6&name=%s)'
                     %(sys.argv[0], urllib.quote_plus(name)))
                     )
                ...
                contextMenu.append(('Adrian TV','XBMC.RunPlugin(%s)' %fav_params))
            liz.addContextMenuItems(contextMenu)
...
#        getData(url,fanart,data)
        ...
        if url:
            ...
            xbmc.executebuiltin("XBMC.Notification(Adrian TV,Failed to extract regex. - "+"this"+",4000,"+icon+")")
    ...
    try:
        ...
        xbmc.executebuiltin("XBMC.Notification(Adrian TV,Please [COLOR yellow]install Youtube-dl[/COLOR] module ,10000,"")")
